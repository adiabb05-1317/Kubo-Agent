  ...
  return (
    <section id="auth" className="glass-panel gradient-border grid gap-8 p-8 md:grid-cols-[360px_auto]">
      ...
      <div className="gradient-border relative flex flex-col gap-6 rounded-3xl p-8">
        ...
        {userEmail ? (
          <p className="text-xs text-[rgba(255,255,255,0.6)]">Signed in as {userEmail}</p>
        ) : null}
      </div>
    </section>
  );
  ...
  return (
    <button
      className={`rounded-full px-4 py-2 font-medium transition ${
        active ? "bg-[rgba(47,128,237,0.25)] text-white" : "bg-transparent hover:bg-[rgba(255,255,255,0.08)]"
      }`}
      onClick={onClick}
    >
      {label}
